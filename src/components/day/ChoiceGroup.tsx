...
export function ChoiceGroup({ events }: { events: ItineraryEvent[] }) {
  return (
    <li className="relative flex gap-4 pb-8 last:pb-0">
      <div className="flex flex-col items-center">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-highlight text-on-accent shadow-sm">
          <Shuffle size={16} />
        </span>
      ...
      <div className="flex-1 pt-1">
        <p className="text-xs font-bold uppercase tracking-wide text-highlight">Pick one</p>
        <div className="mt-1.5 space-y-3">
          {events.map((event, index) => {
            const location = getLocation(event.locationId);
            return (
              <div key={index} className={index > 0 ? "border-t border-line pt-3" : ""}>
